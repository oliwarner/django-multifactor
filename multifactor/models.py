...
class UserKey(models.Model):
    ...
    name = models.CharField(
        max_length=30,
        help_text=_("Easy to remember name to distinguish from any other keys of this sort you own."),
        blank=True,
        null=True,
    )
    ...
    def __str__(self):
        if self.name:
            return _('%(type)s, aka "%(name)s" for %(user)s') % {
                "type": self.get_key_type_display(),
                "name": self.name,
                "user": self.user,
            }
        return _("%(type)s for %(user)s") % {
            "type": self.get_key_type_display(),
            "user": self.user,
        }
